...
from common import (
    SCALE_DEMAND, SCALE_DIV, SCALE_SEQ, SCALE_SUPPLY, STATE_COLOR, TIMESLOT_LABEL,
    apply_filters, load_boundary, page_header, require_analysis, require_data,
...
METRICS = {
    # ── 주 사용자 (나들이) ─────────────────────────────────
    "공영주차 여유도 (천명당 주차면)": dict(col="slots_per_1k", scale=SCALE_SUPPLY,
                                  hint="높을수록 여유", source=None, static=False),
    "혼잡도 (생활인구)": dict(col="living_pop", scale=SCALE_DEMAND,
                        hint="높을수록 붐빔", source=None, static=False),
    "나들이 적합도 (PCA 지수)": dict(col="나들이적합도", scale=SCALE_SEQ,
                             hint="놀거리와 주차를 함께 갖춘 정도", source="suitability", static=True),
    "20~30대 비중": dict(col="young_ratio", scale=SCALE_DEMAND,
                      hint="젊은층이 몰리는 정도", source=None, static=False),
    "음식점 수": dict(col="store_food", scale=SCALE_DEMAND,
                   hint="놀거리 밀집도", source=None, static=True),
    # ── 부 사용자 (확충 진단) ──────────────────────────────
    "실수요 대비 공급 (비아파트 100가구당 주차면)": dict(
        col="slots_per_100_nonapt", scale=SCALE_SEQ,
        hint="낮을수록 실거주 가구에 비해 공영주차가 적음", source=None, static=True),
    "공급 상태 구분": dict(col="공급상태", scale=None, hint="확충 후보·불필요·0면",
                     source="real_demand", static=True, categorical=True),
    "공급 과부족 (생활인구 보정)": dict(col="z_residual", scale=SCALE_DIV, midpoint=0,
                             hint="음수=같은 인구 규모 동네들의 평균보다 적음",
                             source="residual", static=True),
    "아파트 거주 비율": dict(col="apt_ratio", scale=SCALE_SEQ,
                       hint="높을수록 부설주차장이 실수요를 흡수", source=None, static=True),
    "비아파트 가구 (공영주차 실수요)": dict(col="non_apt_households", scale=SCALE_DEMAND,
...
deviation = (mode == "이 시간대의 편차") and not is_static
...
if source:
    ...
    if source == "real_demand":
        ...
        extra.loc[extra.확충필요성 == "불필요", "공급상태"] = "확충 불필요 (아파트 밀집)"
        cols = ["admi_cd", "공급상태"]
    ...
    st.caption("※ 이 지표는 요일·시간대에 따라 변하지 않습니다. "
...
if is_categorical:
    ...
    fig.update_layout(legend=dict(orientation="h", y=-0.02, x=0, title=None))
else:
    fig = px.choropleth_map(
        plot_df, color=color_col,
...
st.caption(
    "공영주차가 0면인 행정동도 지도에 그대로 표시됩니다. "
    "**공급 상태 구분**을 고르면 별도 색으로 확인할 수 있습니다."
)
