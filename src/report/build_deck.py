...
def _content(prs, n, section, title, *, sub=None):
    ...
    if sub:
        # 한 줄짜리 14pt인데 상자를 0.5"로 잡아두면 아래 요소와 경계가 겹친다
        _write(_tf(s, M_LEFT - 0.06, TITLE_Y + 0.72, 11.5, 0.38), [sub],
...
def s_scope(prs, n):
    ...
    **숫자로 확인 가능한 형태**로 적는다. 뒤 슬라이드가 이 숫자를 하나씩 채운다.
    ...
    row(4.55, "목표",
        [[("행정동 424 × 요일 7 × 시간대 6 = ", {}),
          ("17,808행 패널", {"bold": True, "color": INK}), (" 구축", {})],
...
def s_pipeline(prs, n):
    ...
    # 첫 줄이 가장 어려웠던 문제다. 코드가 정상 동작하고 결과도 그럴듯해
    # 에러가 나지 않는 종류라, 스스로 의심해서 확인해야만 드러난다.
    _table(s, [
        ["문제", "해결", "결과"],
        ["설명력이 낮으면 잔차가 '수요로 보정한 값'이 아니게 된다",
         "잔차 순위와 주차면수 순위의 상관을 확인 → 직접 지표로 교체",
         "스피어만 0.997 확인"],
        ["주차장에 행정동 정보가 없고 좌표도 결측(5.7~33.5%)",
         "SGIS 지오코딩으로 주소를 행정동코드로 직접 변환", "배정률 100%"],
        ["데이터마다 행정동 코드 체계가 달랐다",
         "'전국 행정동 코드정보'를 다리로 사용", "조인율 7.5% → 100%"],
    ], y=4.7, col_w=[4.6, 4.4, 2.5], size=12)
...
def s_data(prs, n):
    ...
    _bullets(s, [
        ...
        [("표준데이터를 기본으로 하고 0면인 동에만 보충해 중복 없이 ", {}),
         ("78,267면 / 358개 동", {"bold": True, "color": INK}), ("을 확보했다", {})],
    ], y=5.75)


def s_join(prs, n):
    """전처리에서 가장 판단이 어려웠던 문제.

    데이터를 '붙이는' 일(코드 크로스워크·지오코딩)은 손이 많이 갈 뿐 정답이 있다.
    반면 공영주차 0면인 동을 어떻게 다룰지는 **지표마다 답이 반대**라 정답이 없었다.
    그래서 이 자리에는 붙이기 대신 이 판단을 둔다.
    """
    s = _content(prs, n, "활용 데이터", "원본 7종을 하나의 표로",
                 sub="전처리 — 손이 가장 많이 간 단계이자, 정답이 없는 판단이 나온 곳")
    _table(s, [
        ["단계", "무엇을 했나", "결과"],
        ["코드 통일", "데이터마다 행정동 코드 체계가 달라 '전국 행정동 코드정보'를 다리로 놓음",
         "조인율 7.5% → 100%"],
        ["주차장 배정", "행정동 정보가 없고 좌표도 결측 → 주소를 SGIS 지오코딩",
         "배정률 100% (856/856)"],
        ["공급 병합", "표준데이터를 기본으로, 0면인 동에만 서울시 API 보충",
         "0면 109 → 66개 동"],
        ["거처 결합", "총조사는 가구 적은 소분류를 비공개(X) → 합산 대신 차감",
         "결측 11 → 2개 동"],
        ["패널화 · 검증", "요일×시간대 집계 · 공휴일 제외 · 파생변수 · 자동 품질검사",
         "17,808행 × 31열"],
    ], y=2.3, col_w=[2.0, 6.9, 2.6], size=12.5, row_h=0.44)
    # 표 아래 4.94"부터. 불릿은 한 줄에 들어가야 표를 덮지 않는다
    _bullets(s, [
        [("가장 어려웠던 건 붙이는 일이 아니라 ", {}),
         ("0을 어떻게 다룰지", {"bold": True, "color": INK}),
         ("였다 — 회귀에선 빼고 안내에는 넣었다", {})],
        [("문정2동은 음식점 821개에 공영주차 0면 — 빼면 ", {}),
         ("가장 먼저 알아야 할 동네가 사라진다", {"bold": True, "color": INK})],
        [("빼면 상관이 강해 보이므로 포함·제외를 병기했다 ", {}),
         ("(음식점 -0.02 → +0.03, 부호 반전)", {"bold": True, "color": RED})],
    ], y=5.1)
    _note(s, "자동 검증 5종 — 구조·중복·논리·결측·이상치. 이상치는 제거하지 않았다(실제 번화가라서)")
...
def s_eda(prs, n):
    ...
    _note(s, "EDA가 바꾼 결정 — 공휴일 제외 · 시간대 6구간(아침은 추천 제외) · 0면 동 보충 · 민감도 병기", y=6.75)
...
def s_learned(prs, n):
    ...
    for title, desc in items:
        ...
        y += 1.42
    _note(s, "산출물 — 보고서 reports/report.md · 그림 19종 · 데이터 8종 · 대시보드 6페이지", y=6.75)
